...
public class Rook extends ChessFigure {
	...
	public Rook(boolean team, int col, int row, FieldRectangle[][] rectArray, GlowField[][] glowArray, Pane fieldPane,
		...
		// invalid.");
		// }
		this.icon = new ImageView("" + team + "rook.png");
		this.type = 4;
		...
		this.icon.hoverProperty().addListener(e->{
			if((ChessFieldController.getActiveTeam() == this.team && this.icon.isHover()))
				this.icon.setEffect(new ImageInput(new Image(""+team+"rooksel.png"),icon.getX(),icon.getY()));
			else 
	...
	public void canHit(){
		/*
		 * Not in use in Tower class because hit-pattern = move-pattern
		 */
	}
	...
	public void canMove() {
		 ...
		 */
		while(col+count <= 7){
			int breaker = glowArray[col+count][row].determineShow(team, col+count, row, rectArray);
			if(breaker != 0){
				break;
			}
			count++;
		...
		count=1;
		while(row+count <= 7){
			int breaker = glowArray[col][row+count].determineShow(team, col, row+count, rectArray);
			if(breaker != 0){
				break;
			}
			count++;
