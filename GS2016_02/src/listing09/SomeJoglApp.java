	...
	static class GLEventListenerImpl implements GLEventListener {
		...
		public void init(GLAutoDrawable drawable) {
			if (drawable.getGL().isGL3()) {
				...
				float[] colorData = new float[36 * 3];
				...
				vertices.rewind();
				FloatBuffer colors = Buffers.newDirectFloatBuffer(colorData);
				colors.rewind();

				gl.glBindVertexArray(VAO[0]);
				gl.glBindBuffer(GL_ARRAY_BUFFER, VBO[0]);
				gl.glBufferData(GL_ARRAY_BUFFER, vertices.capacity() * Buffers.SIZEOF_FLOAT, vertices,
						GL_STATIC_DRAW);
				...
				gl.glBufferData(GL_ARRAY_BUFFER, colors.capacity() * Buffers.SIZEOF_FLOAT, colors,
						GL_STATIC_DRAW);
				gl.glEnableVertexAttribArray(locColor);
				gl.glVertexAttribPointer(locColor, 3, GL_FLOAT, false, 3 * Buffers.SIZEOF_FLOAT, 0L);
