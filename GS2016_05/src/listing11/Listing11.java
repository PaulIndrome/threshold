...
import tools.shader.texture.ColorAndNormalMapShaderPipeline;
...
public class Listing11 {
	...
	static class GLEventListenerImpl implements GLEventListener, KeyListener {
		...
		GL3 gl;
		ColorAndNormalMapShaderPipeline pipeline;
		...
		public void init(GLAutoDrawable drawable) {
			if (drawable.getGL().isGL3()) {
				...
				pipeline = new ColorAndNormalMapShaderPipeline(gl, new PMVMatrix(), new Material(), new Light());
				pipeline.init();
				if (!pipeline.isReady()) {
				...
				gl.glEnableVertexAttribArray(pipeline.getLocTexCoord0()); // color map coords
				gl.glVertexAttribPointer(
				...
				gl.glEnableVertexAttribArray(pipeline.getLocTexCoord1()); // normal map coords - same as color map in this case
				gl.glVertexAttribPointer(
						pipeline.getLocTexCoord1(), 
						2, 
						GL_FLOAT, 
						false, 
						wf.getVertexFormat().getComponentCount() * Buffers.SIZEOF_FLOAT, 
				...
				// load and specify textures
				gl.glGenTextures(textures.length, textures, 0);
				gl.glEnable(GL_TEXTURE_2D);
				createAndBindTextureObject(textures[0], TextureImage.EARTH_COLOR);
				createAndBindTextureObject(textures[1], TextureImage.EARTH_NORMAL);
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				if (GLProfile.isAvailable(GLProfile.GL3)) {
					...
					AnimatedJOGLFrame.show("Color And Normal Mapping (OpenGL 3 Core)...", caps, listener, listener, 600, 600);
