...
import tools.shader.gouraud.ADSReflectionShaderPipeline;
...
public class Listing06 {
	...
	static class GLEventListenerImpl implements GLEventListener, KeyListener {
		...
		GL3 gl;
		ADSReflectionShaderPipeline pipeline;		
		...
		public void init(GLAutoDrawable drawable) {
			if (drawable.getGL().isGL3()) {
				...
				pipeline = new ADSReflectionShaderPipeline(gl, new PMVMatrix(), new Material(), new Light());
				pipeline.init();
				if (!pipeline.isReady()) {
	...
	public static final String TITLE = "ADS Shader using Reflection (OpenGL 3 Core)...";
